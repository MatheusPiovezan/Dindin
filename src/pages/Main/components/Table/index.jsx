...
function Table() {
  ...
  useEffect(() => {
    ...
    asc
      ? localTransaction.sort((a, b) => new Date(a.data) - new Date(b.data))
    ...
    <T.Container>
      <div className="titles">
        <div className="small date" onClick={() => setAsc(!asc)}>
          <strong>Data</strong>
          <img src={asc ? IconPolygonUp : IconPolygonDown} alt="" />
        </div>
      ...
      {orderTransact.map((transaction) => (
        <div className="table" key={transaction.id}>
          <strong className="small date">{dateFormat(transaction.date)}</strong>
          <span className="middle">{dayFormat(transaction.date)}</span>
          <span className="big">{transaction.description}</span>
          <span className="big">{transaction.category_name}</span>
          <Strong className="small value" valuecolor={transaction.type}>
          ...
          <div className="edit-delete">
            <div>
              <img
                ...
                onClick={() => {
                  {
                    setOpenModalEditTransact(true);
                  }
                  {
                    setItem("idTransact", transaction.id);
                  }
                }}
                alt=""
              />
